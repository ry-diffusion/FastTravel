    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="max-w-[560px]">
        <DialogHeader>
          ...
          <DialogTitle className="text-base">{diag.title}</DialogTitle>
          {contextLabel && (
        ...
        <div className="flex flex-col gap-4 py-2 max-h-[60vh] overflow-y-auto">
          {/* Summary */}
          <p className="text-sm text-muted-foreground leading-relaxed">{diag.summary}</p>

          {/* Suggestions */}
          {diag.suggestions.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold text-muted-foreground">What to try</span>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-semibold text-muted-foreground">Raw error</span>
            <pre className="bg-muted rounded-md p-3 text-xs font-mono whitespace-pre-wrap break-words leading-relaxed max-h-44 overflow-y-auto">
              {raw}
            </pre>
          </div>
        </div>
